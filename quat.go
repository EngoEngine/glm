...
func Mat4ToQuat(m *Mat4) *Quat {
	...
	if tr := m[0] + m[5] + m[10]; tr > 0 {
		s := 0.5 / float32(math.Sqrt(float64(tr+1.0)))
		return &Quat{
			0.25 / s,
			Vec3{
				(m[6] - m[9]) * s,
	...
	if (m[0] > m[5]) && (m[0] > m[10]) {
		s := 2.0 * float32(math.Sqrt(float64(1.0+m[0]-m[5]-m[10])))
		return &Quat{
			(m[6] - m[9]) / s,
			Vec3{
				0.25 * s,
	...
	if m[5] > m[10] {
		s := 2.0 * float32(math.Sqrt(float64(1.0+m[5]-m[0]-m[10])))
		return &Quat{
			(m[8] - m[2]) / s,
			Vec3{
				(m[4] + m[1]) / s,
	...
	s := 2.0 * float32(math.Sqrt(float64(1.0+m[10]-m[0]-m[5])))
	return &Quat{
		(m[1] - m[4]) / s,
		Vec3{
			(m[8] + m[2]) / s,
